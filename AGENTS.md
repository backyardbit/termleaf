...
- **Keyboard in the style of vim, plus the mouse.**
  ...
  - Pinch-to-zoom is opt-in (`--pinch`) and lives in `pinch.rs`. No terminal forwards pinches, so termleaf reads them from the OS: a listen-only CoreGraphics event tap on macOS (`pinch/macos.rs`, reading the undocumented gesture fields 110/113/132), and raw `/dev/input` touchpad records on Linux (`pinch/linux.rs`). It needs Input Monitoring for the terminal app, or the `input` group. Keep it opt-in and keep the decoding pure (`pinch/gesture.rs`, `pinch/evdev.rs`) so it stays tested; real gestures can't be exercised in CI. Pinches only count while terminal focus reporting says the pane has focus.
