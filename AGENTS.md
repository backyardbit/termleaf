...
- **Scrolling never re-sends pixels.** Each page is cut into tiles of at most 64×48 cells at the current scale (`layout.rs`). Each tile is rendered once, transmitted once as a Kitty virtual placement, and cached. Scrolling and panning only rewrite the Unicode placeholder cells, with row and column diacritics picking the visible slice of each tile. That is what keeps scrolling smooth under herdr. A scroll is shown at once with whatever tiles are already cached, never held back until the tiles it uncovers have rendered; the panes above and below are prefetched behind the visible tiles. While the zoom is changing (and for 150 ms after the last change), nothing new is rendered: the tiles already in the terminal are redisplayed stretched through a second Kitty placement (placement id 2; normal tiles use id 1, and placeholder cells name their placement in the underline colour). Once zooming settles, tiles are rendered at the final scale and replace the stretched ones when all visible tiles have arrived.
